  ...
  sidebarMenu(
    menuItem("Overview", tabName = "overview", icon = icon("dashboard"),
             ...
             sliderInput("energy", "How energetic?",
                         0, 1, value = c(0,1), step = 0.01),
             sliderInput("tempo", "How fast? (in BPM)",
                         0, 200, c(0,150), step = 1),
...
body <- dashboardBody(
  tabItems(
             tabItem("overview",
                     fluidRow(
                       column(11,
                              box(title = "Get Your Saved Songs", status = "warning", solidHeader = TRUE, width = 1200,
                                  a(htmlOutput("auth"), href=htmlOutput("auth"), target="_blank"),
                                  br(),
                                  textInput("url", "2. Once you've logged in and reached the Spotify homepage, paste the full URL from your browser (it should contain a code): "),
